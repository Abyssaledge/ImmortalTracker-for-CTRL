...
def save_to_bin(tracklet_dict, save_path, prefix=None):
    ...
    print(f'Num off output objects: {len(objects.objects)}')
...
def worker(cfg, tracklet_dict_1, tracklet_dict_2, segnames, out_tracklet_dict, counter_list, token, process):
    import torch
    try:
        torch.cuda.set_device(token % 8)
        for i, segment_name in enumerate(segnames):
            if i % process != token:
                continue
            trks_1 = tracklet_dict_1[segment_name]
            ...
            counter_list.append(segment_name)
            print(f'Finish {len(counter_list)} / {len(segnames)}')
...
if __name__ == '__main__':
    ...
    save_path = osp.join(osp.dirname(bin_path_1), save_name + '_mp.bin')
    ...
    out_tracklet_dict = manager.dict()
    beg = time.time()
    segnames = list(tracklet_dict_1.keys())

    if args.process > 1:
        pool = mp.Pool(args.process)
        for token in range(args.process):
            result = pool.apply_async(worker, args=(cfg, tracklet_dict_1, tracklet_dict_2, segnames, out_tracklet_dict, counter_list, token, args.process))
        pool.close()
        pool.join()
    else:
        worker(cfg, tracklet_dict_1, tracklet_dict_2, segnames, out_tracklet_dict, counter_list, 0, 1)
